    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, Animal> {
        ...
        return try {
            ...
            if (response.status.isSuccess()) {
                ...
                val mappedResponse = JsonConfig.json.decodeFromString<AllAnimalsNetwork>(responseBody)
                val nextKey = if (mappedResponse.pagination.current_page == mappedResponse.pagination?.total_pages) null else startKey + 1
                ...
                LoadResult.Error(Throwable("Request failed with status: ${response.status.value}"))
            }
        } catch (e: Exception) {
            LoadResult.Error(Throwable(e.localizedMessage ?: "Something went wrong"))
        }
    }
}
